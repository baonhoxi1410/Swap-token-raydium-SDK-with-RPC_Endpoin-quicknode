...
export class RaydiumSwap {
  ...
  private getSwapSide(
    ...
    if (poolKeys.baseMint.equals(wantFrom) && poolKeys.quoteMint.equals(wantTo)) {
      ...
      return "out";
    } else {(!poolKeys || !poolKeys.baseMint || !poolKeys.quoteMint)
      throw new Error(`Not suitable pool fetched. Want to swap from ${wantFrom} to ${wantTo}, but pool has baseMint: ${poolKeys.baseMint} and quoteMint: ${poolKeys.quoteMint}`);
    }
  }
  ...
  async getSwapTransaction(
    ...
    const currencyIn = (swapSide === "in" || swapSide === "out") ? baseToken : quoteToken;
const currencyOut = (swapSide === "in" || swapSide === "out") ? quoteToken : baseToken;
  ...
  //       skipPreflight: true,  // Skip preflight checks if needed
  ...
  //     // Retry logic for expired blockhash
  //     let latestValidBlockHeight = latestBlockhash.lastValidBlockHeight;
  //     let currentBlockHeight = await this.connection.getBlockHeight();
      
  //     // Ensure lastValidBlockHeight is not too outdated
  //     if (currentBlockHeight > latestValidBlockHeight) {
  //       console.warn("Transaction might expire soon. Retrying with fresh blockhash...");
  //       // Optionally, re-send or handle this case depending on your logic
  //       return this.sendLegacyTransaction(tx);  // Retry logic (recurse or handle appropriately)
  //     }
  ...
  //     // Confirm the transaction manually using confirmTransaction
  //     const confirmation = await this.connection.confirmTransaction(confirmationStrategy, 'confirmed');
  
  //     // Check if the confirmation contains an error
  ...
  //     }
  //     throw new Error('Transaction failed with an unknown error');
  //   }
  // }
  async sendLegacyTransaction(tx: Transaction): Promise<string> {
    const signature = await this.connection.sendTransaction(tx, [this.wallet.payer], {
...
const confirmation =
 await this.connection.confirmTransaction(signature,'confirmed',); // Increase timeout to 60 seconds
    if (confirmation.value.err) {
      console.error('Transaction error details:', confirmation.value.err);

      const errorDetails = JSON.stringify(confirmation.value.err, null, 2); // Indented for better readability
